...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            note = 'Pizza order has failed. Try again.' #yla filled_form is not valid te3tina hadi
...
def edit_order(request, pk):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            filled_form.save()
            form = filled_form
            note = 'Order has been updated.' # hadi tekhrejleh mor ma ydir user save of his new order
